...
### 3. 풀이
```
1. 좌표를 뒤집어주는 함수를 정의해 준다. 3X3크기의 부분 행렬을 뒤집어야 하므로 좌표+3보다 작을때까지 루프한다.

2. 행렬 A, B의 요소를 (0,0)부터 확인해준다. 이 때, 오른쪽 끝에서 2칸 이상 떨어진 곳 까지 뒤집을 수 있으니
   모두 x,y좌표를 좌표-3 보다 작거나 같을때까지 루프해서 확인한다.

3. 뒤집기가 끝나면 A,B가 같아졌는지 확인하고, 같게 만들 수 있으면 뒤집은 횟수, 같지 않다면 -1을 출력해준다.
```
